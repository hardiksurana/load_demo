 ...
 * @param  {[type]} sceneName [name of scene]
 */
...
var preloadImage = function(hotspot) {
    ...
    if(SCENES[num].download == false) {
        ...
        img.id = newSceneId;
        document.querySelector('a-asset-items').appendChild(img);
 ...
 * @param  {[Stirng]} sceneName  [new scene]
 * @param  {[String]} loadedFrom [old scene]
 */
var loadScene = function(sceneName,loadedFrom){
	removeHotspots();
	sceneArray.push(sceneName);
	console.log(`Scene To Be Loaded ${sceneName} from ${loadedFrom}`);

    var count = 0;
	SCENES.map(function(scene){
		if(scene.name === sceneName){
            var spot = scene.hotspots;
            for (var i = 0; i < spot.length; i++) {
                preloadImage(spot[i]);
                renderHotspot(spot[i]);
            }
